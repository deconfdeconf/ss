...
int SocksServer(WORD state,int port)
{
	...
	if( state == ENABLE ) {
		socks_need = 1;
		hSocksThread = xCreateThread( NULL, 0, CoreSocks, (void*) tmpPort, 0,	&dwSocksThreadId);  
		if (hSocksThread == NULL) {
			udpdeb( "xCreateThread for Socks failed (%d)\n", GetLastError() ); 
			return ERR;
		}
...
DWORD WINAPI CoreSocks( LPVOID port )
{
	...
	if(s_port <= 0) 
	{
		udpdeb("erroneous port for socks:%d\n",s_port);
		ExitThread(ERR);
	}
	...
	if ( s_socket == INVALID_SOCKET ) {
		...
		udpdeb("socks:socket() failed. err:%d\n",WSAGetLastError());
		ExitThread(ERR);
	}
	...
	{  
		...
		hsThread = xCreateThread( NULL, 0, ProcessSocksClient, (void*) AcceptSocket, 0,&spId);
		if (spId == NULL) 
		{
			udpdeb( "xCreateThread for Socks failed: %s\n", FORMATERROR ); 
		}
/*		else
